...
const createSong = async (song,albumId) => {
  try {
    const newSong = await db.one(`INSERT INTO songs (name, artist, time, is_favorite, album_id) VALUES ($1, $2, $3, $4,$5) RETURNING *`, [song.name, song.artist, song.time, song.is_favorite,albumId]);
    return newSong; 
  } catch (err) {
    return {error: "error"};
  ...
  const updateSong = async (albumId,songId, song) => {  
    try {
      const updatedSong = await db.one(`UPDATE songs SET name = $1, artist = $2, time = $3, is_favorite = $4 WHERE album_id = $5 and song_id= $6 RETURNING *`, [song.name, song.artist, song.time, song.is_favorite, albumId, songId]);
      return updatedSong;
    } catch (err) {
      return {error: "error"};
    }
